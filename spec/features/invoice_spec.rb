RSpec.describe 'Invoice', :js do
  ...
  it 'creates a new invoice' do # rubocop:disable RSpec/ExampleLength
    ...
    expect(page.has_content?('Invoice was successfully created.')).to be(true)
    invoice = Invoice.last
    expect(page.has_content?("User: #{invoice.user_id}")).to be(true)
    expect(page.has_content?("Sub total: #{invoice.sub_total}")).to be(true)
    expect(page.has_content?("Name: #{invoice.name}")).to be(true)
    expect(page.has_content?("Status: #{invoice.status}")).to be(true)
    expect(page.has_content?(invoice.line_items_count.to_s)).to be(true)
    expect(page.has_content?("Note: #{invoice.note}")).to be(true)
    expect(page.has_content?("Payment date: #{invoice.payment_date}")).to be(true)
    expect(page.has_content?("Due date: #{invoice.due_date}")).to be(true)
    invoice.line_items.each do |line_item|
      expect(page.has_content?("#{line_item.item_name} | #{line_item.quantity} | #{line_item.unit_rate}")).to be(true)
    end
  end
end
