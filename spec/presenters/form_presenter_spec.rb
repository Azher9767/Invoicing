RSpec.describe FormPresenter do
  ...
  context 'form render with validation errors' do
    ...
    it "error class with options" do
      html_msg = presenter.field_options(:name, 'form-select',
        prompt: 'Select Line Item',
        multiple: true,
        data: {invoice_target: 'product'},
        id: 'product_id')
     
      expected_msg =  {:aria=>{:describedby=>"name_aria"}, :class=>"form-control form-select is-invalid", :data=>{:invoice_target=>"product"}, :id=>"product_id", :multiple=>true, :prompt=>"Select Line Item"}
      expect(html_msg).to eq(expected_msg)
    end
  end
end
